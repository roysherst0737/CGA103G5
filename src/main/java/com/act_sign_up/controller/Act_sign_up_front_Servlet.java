...
//		}
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			/*********************** 1.接收請求參數 - 輸入格式的錯誤處理 *************************/
			Timestamp sign_up_begin_time = Timestamp.valueOf(req.getParameter("sign_up_begin_time").trim());
			Timestamp sign_up_end_time = Timestamp.valueOf(req.getParameter("sign_up_end_time").trim());
			...
			if (sign_up_begin_time.compareTo(nowTime) < 0 && sign_up_end_time.compareTo(nowTime) > 0) {
				act_sign_upVO = act_sign_upSvc.addAct_sign_up(act_no, mem_no, accompany_count);
				String url = "act-detail.jsp?" + act_no;
				res.sendRedirect(url);
			}else {
				errorMsgs.add("不在報名期間");
			}
			
			if (!errorMsgs.isEmpty()) {
				RequestDispatcher failureView = req.getRequestDispatcher("/front-end/act/act-detail.jsp?" + act_no);
				failureView.forward(req, res);
				return;
			}
