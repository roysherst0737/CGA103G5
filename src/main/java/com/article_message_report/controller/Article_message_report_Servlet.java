...
//			String rpt_msg_content = req.getParameter("rpt_msg_content");
//			String rpt_msg_contentReg = "^[(\u4e00-\u9fa5)(a-zA-Z0-9_)]{2,10}$";
//			if (rpt_msg_content == null || rpt_msg_content.trim().length() == 0) {
//				errorMsgs.add("員工姓名: 請勿空白");
//			} else if(!rpt_msg_content.trim().matches(rpt_msg_contentReg)) { //以下練習正則(規)表示式(regular-expression)
...
//	        }
			...
			String msg_note = req.getParameter("msg_note").trim();
				if (msg_note == null || msg_note.trim().length() == 0) {
					errorMsgs.add("處理註記請勿空白");
				}	
