...
public class Article_message_report_Servlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest req, HttpServletResponse res) throws ServletException, IOException {
		...
		if ("getOne_For_Display".equals(action)) { // 來自select_page.jsp的請求
			...
			Article_message_report_VO article_message_report_VO = article_message_report_Svc
					.getOneArticle_message_report(art_msg_rpt);
		...
		if ("update".equals(action)) { // 來自update_emp_input.jsp的請求
			...
			if (!errorMsgs.isEmpty()) {
				...
				RequestDispatcher failureView = req.getRequestDispatcher(
						"/back-end/article_message_report/update_article_message_report_input.jsp");
				failureView.forward(req, res);
				return;
			}
		...
		if ("insert".equals(action)) { // 來自addEmp.jsp的請求
			...
			/*************************** 2.開始新增資料 ***************************************/
			Article_message_report_Service article_message_report_Svc = new Article_message_report_Service();
			article_message_report_VO = article_message_report_Svc.addArticle_message_report(mem_no, art_msg_no,
					rpt_time, rpt_msg_content);
