...
public class Article_message_report_DAO implements Article_message_report_DAO_interface {
	...
	private static final String INSERT_STMT = "INSERT INTO article_message_report (mem_no,art_msg_no,rpt_time,rpt_msg_content) VALUES (?, ?, ?, ?)";
	private static final String GET_ALL_STMT = "SELECT art_msg_rpt,mem_no,art_msg_no,rpt_time,rpt_msg_content,mng_no,msg_done_time,msg_status,msg_result,msg_note FROM article_message_report order by art_msg_rpt";
	private static final String GET_ONE_STMT = "SELECT art_msg_rpt,mem_no,art_msg_no,rpt_time,rpt_msg_content,mng_no,msg_done_time,msg_status,msg_result,msg_note FROM article_message_report where art_msg_rpt = ?";
	private static final String DELETE = "DELETE FROM article_message_report where art_msg_rpt = ?";
	private static final String UPDATE = "UPDATE article_message_report set mem_no = ?,art_msg_no = ?,rpt_time = ?,rpt_msg_content = ?,mng_no = ?,msg_done_time = ?,msg_status = ?,msg_result = ?,msg_note = ? where art_msg_rpt = ?";
	private static final String CHECK = "UPDATE article_message_report set mng_no = ?,msg_done_time = ?,msg_status = ?,msg_result = ?,msg_note = ? where art_msg_rpt = ?";
	...
	public void update(Article_message_report_VO article_message_report_VO) {
		...
		try {
			...
			pstmt.setInt(10, article_message_report_VO.getArt_msg_rpt());
			pstmt.executeUpdate();
	...
	public Article_message_report_VO findByPrimaryKey(Integer art_msg_rpt) {
		...
		try {
			...
			while (rs.next()) {
				...
				article_message_report_VO.setMsg_status(rs.getInt("msg_status"));
	...
	public void check(Article_message_report_VO article_message_report_VO) {
		...
		try {
			...
			pstmt.setInt(1, article_message_report_VO.getMng_no());
			pstmt.setTimestamp(2, article_message_report_VO.getMsg_done_time());
			pstmt.setInt(3, article_message_report_VO.getMsg_status());
			pstmt.setInt(4, article_message_report_VO.getMsg_result());
			pstmt.setString(5, article_message_report_VO.getMsg_note());
			pstmt.setInt(6, article_message_report_VO.getArt_msg_rpt());
			pstmt.executeUpdate();
