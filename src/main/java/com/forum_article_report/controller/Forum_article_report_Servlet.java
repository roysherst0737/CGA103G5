...
//				
			...
			if (rpt_note == null || rpt_note.trim().length() == 0) {
				errorMsgs.add("文章處裡註記請勿空白");
			}	
			...
			forum_article_report_VO.setRpt_note(rpt_note);
				...
				/***************************2.開始修改資料*****************************************/
				Forum_article_report_Service forum_article_report_Svc = new Forum_article_report_Service();
				forum_article_report_VO = forum_article_report_Svc.updateForum_article_report(frm_art_rpt_no, mem_no, frm_art_no, rpt_time, rpt_content,mng_no, rpt_done_time, rpt_status, rpt_result, rpt_note);
        ...
        if ("insert".equals(action)) { // 來自addEmp.jsp的請求  
			...
			forum_article_report_VO.setRpt_note(rpt_note);
				...
				if (!errorMsgs.isEmpty()) {
					req.setAttribute("forum_article_report_VO", forum_article_report_VO); // 含有輸入格式錯誤的empVO物件,也存入req
					RequestDispatcher failureView = req
							.getRequestDispatcher("/back-end/forum_article_report/addForum_article_report.jsp");
					failureView.forward(req, res);
					return;
				}
				...
				/***************************2.開始新增資料***************************************/
				Forum_article_report_Service forum_article_report_Svc = new Forum_article_report_Service();
				forum_article_report_VO = forum_article_report_Svc.addForum_article_report(mem_no, frm_art_no, rpt_time, rpt_content,mng_no, rpt_done_time, rpt_status, rpt_result, rpt_note);
