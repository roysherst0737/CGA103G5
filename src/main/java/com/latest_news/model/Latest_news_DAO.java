...
public class Latest_news_DAO implements Latest_news_DAO_interface{
	...
	private static final String INSERT_STMT = 
			"INSERT INTO latest_news (news_content,news_status,news_time) VALUES (?, ?, ?)";
		private static final String GET_ALL_STMT = 
			"SELECT latest_news_no,news_content,news_status,news_time FROM latest_news order by latest_news_no";
		private static final String GET_ONE_STMT = 
			"SELECT latest_news_no,news_content,news_status,news_time FROM latest_news where latest_news_no = ?";
		private static final String DELETE = 
			"DELETE FROM latest_news where latest_news_no = ?";
		private static final String UPDATE = 
			"UPDATE latest_news set news_content = ?,news_status = ?,news_time = ? where latest_news_no = ?";
	...
	public void insert(Latest_news_VO latest_news_VO) {
		...
		try {
			...
			pstmt.setTimestamp(3, latest_news_VO.getNews_time());
	...
	public void update(Latest_news_VO latest_news_VO) {
		...
		try {
			...
			pstmt.setInt(2, latest_news_VO.getNews_status());
			pstmt.setTimestamp(3, latest_news_VO.getNews_time());
			pstmt.setInt(4, latest_news_VO.getLatest_news_no());
			pstmt.executeUpdate();
	...
	public Latest_news_VO findByPrimaryKey(Integer latest_news_no) {
		...
		try {
			...
			while (rs.next()) {
				...
				latest_news_VO.setNews_time(rs.getTimestamp("news_time"));
