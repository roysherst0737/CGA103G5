...
public class Pub_pics_DAO implements Pub_pics_DAO_interface {
	...
	@Override
	public void insert(Pub_pics_VO pub_pics_VO) {

		try (Connection con = ds.getConnection(); PreparedStatement pstmt = con.prepareStatement(INSERT_STMT)) {
			pstmt.setInt(1, pub_pics_VO.getPub_no());
			pstmt.setBlob(2, pub_pics_VO.getPub_pic());
			pstmt.executeUpdate();
	...
	public void update(Pub_pics_VO pub_pic_VO) {
		...
		try (Connection con = ds.getConnection(); PreparedStatement pstmt = con.prepareStatement(UPDATE)) {
			...
			pstmt.setInt(3, pub_pic_VO.getPub_pic_no());
			pstmt.executeUpdate();
