						...
						<div class="modal-content ">
							...
							<div class="modal-body container">
								<div class="row d1">
									...
									<div id="oc`+ i + `"class="col-6" style="padding: 10px;visibility: hidden;">
									<div >
...
function radio(but, e) {
	if (but.value == "自選") {
		document.querySelector('#oc' + e).style.visibility = '';
	} else {
		document.querySelector('#oc' + e).style.visibility = "hidden";
	}
}
...
function close_dialog() {
	...
	document.querySelectorAll('#exampleModal1, #exampleModal2, #exampleModal3, #exampleModal4, #exampleModal5, #exampleModal6, #exampleModal7').forEach(e => { e.setAttribute("aria-hidden", "true") });
...
const blist = document.querySelectorAll('#b1,#b2,#b3,#b4,#b5,#b6,#b7');
blist.forEach((e, index) => {
	e.addEventListener('click', function myb(bt) {
		const model = document.querySelector('#exampleModal' + (parseInt(index) + 1));
		model.classList.add("show");
		model.style.display = "block";
		document.querySelector('body').classList.add("modal-open");
		console.log(123);
		document.querySelector('#open_24_' + (parseInt(index) + 1)).focus();
	})
});
...
function aa(open) {
	...
	function getOpenTimeSetButton(m, index) {
		if (m.indexOf(1) == -1) {
			...
			document.querySelector('#oc' + index).style.visibility = '';
