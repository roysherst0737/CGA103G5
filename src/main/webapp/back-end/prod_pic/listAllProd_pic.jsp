...
<table>
	...
	<c:forEach var="prod_picVO" items="${list}" begin="<%=pageIndex%>" end="<%=pageIndex+rowsPerPage-1%>">
		...
		<tr>
			...
			<td>${prod_picVO.prod_pic}</td>
			<td>${prod_picVO.prod_pic_name}</td>
			<td>
			  <FORM METHOD="post" ACTION="<%=request.getContextPath()%>/back-end/prod_pic/prod_pic.do" style="margin-bottom: 0px;">
			     <input type="submit" value="修改">
			     <input type="hidden" name="prod_pic_no"  value="${prod_picVO.prod_pic_no}">
